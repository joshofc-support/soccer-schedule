...
- **月ごとのシート（タブ）はすべて自動で読み込みます。** シートを追加すれば（例：来月分）そのまま反映され、月送りボタンで全期間を閲覧できます。
- **年の決まり方（優先順位）**：
  1. **A列のセルが日付データなら、その年をそのまま使います（これが正）。** 表示形式が「6/1」でも、セルの実体（例：`2026/06/01`）の年で読み込みます。
  2. セルが文字列（例：手打ちの「6/1」）の場合は、シート名の西暦（例：`2027年4月`）を使います。
  3. どちらも無ければ `YEAR`（年度：4月〜翌3月、1〜3月は翌年扱い）で補完します。
- **次年度も同じファイルで運用できます。** シートを追加しても、今のシートの日付を翌年に書き換えても、A列が日付データである限り自動で正しく反映されます。コード変更は不要です。
- ファイル名（スプレッドシートのタイトル）は自由に変更して構いません。カレンダーはファイル名を使っていないため、影響しません。
